...
function mention(users, userMap) {
    return users.map(u => userMap[u] ? `<@${userMap[u]}>` : u).join(" ");
}
...
(async () => {
    ...
    if (eventType === "opened" || eventType === "reopened") {
        ...
        if (reviewers.length > 0) {
            ...
            const msg = `🕵️ Reviewer assignment for PR ${prLink} in \`${repo}\`: ${mention(reviewers, userMap)}`;
    ...
    if (eventType === "review_request_removed" && removedReviewer) {
        ...
        if (toAdd) {
            ...
            const msg = `⚠️ Reviewer ${mention([removedReviewer], userMap)} was removed from PR ${prLink} in \`${repo}\`. Replaced by ${mention([toAdd], userMap)}.`;
            await notifySlack(msg);
        }
    }
})();
