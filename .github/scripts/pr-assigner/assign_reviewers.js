        ...
        if (needed > 0) {
            ...
            if (toAdd.length > 0) {
                ...
                if (addedReviewers.length > 0) {
                    ...
                    if (eventType === "review_request_removed" && removedReviewer) {
                        msg = `⚠️ Reviewer ${mention([removedReviewer], userMap)} was removed from PR #${prNumber} ${prLink} in \`${repo}\`. Replaced by ${mention(addedReviewers, userMap)}.`;
            ...
            if (removedReviewer) {
                console.log(`Reviewer ${removedReviewer} removed from PR #${prNumber}, but enough reviewers (${currentReviewers.length}) still remain. No new assignment needed.`);
            } else {
