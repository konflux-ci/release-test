...
function mention(users, userMap) {
    return users.map(u => userMap[u] || u).join(" ");
}
...
(async () => {
    ...
    if (eventType === "opened" || eventType === "reopened") {
        ...
        if (reviewers.length > 0) {
            ...
            const msg = `🕵️ Reviewer assignment for PR: “<${prUrl}|${prTitle}>” → ${mention(reviewers, userMap)}`;
    ...
    if (eventType === "review_request_removed" && removedReviewer) {
        ...
        if (toAdd) {
            ...
            const msg = `⚠️ Reviewer ${mention([removedReviewer], userMap)} was removed from PR: “<${prUrl}|${prTitle}>”. Replaced by ${mention([toAdd], userMap)}.`;
            await notifySlack(msg);
        }
    }
})();
