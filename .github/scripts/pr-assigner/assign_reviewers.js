...
async function assignReviewers(owner, repo, pull_number, excludedUsers) {
    ...
    console.log(userMap)
    const eligibleUsers = Object.entries(userMap.users)
        .filter(([_, meta]) => meta.assign !== false)
...
async function main() {
    let payload;
    // Check if running in local environment, mock the payload for local testing
    if (process.env.GITHUB_EVENT_PAYLOAD) {
        payload = JSON.parse(process.env.GITHUB_EVENT_PAYLOAD);
    } else {
        // Mock payload for local testing (replace with your own mock data)
        payload = {
            action: "opened",
            pull_request: {
                number: 18,
                requested_reviewers: [{ login: "johnbieren" }, { login: "FilipNikolovski" }],
                user: { login: "davidmogar" },
                draft: false,
            },
            repository: { owner: { login: "konflux-ci" }, name: "release-test" },
        };
    }
